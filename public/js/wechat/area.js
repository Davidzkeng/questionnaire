/*
*	全国三级城市联动 js版 代码整理：懒人之家 www.lanrenzhijia.com
*/
function Dsy() {
    this.Items = {};
}
...
function change(level) {
    ...
    for (index = 0; index < level; index++) {
        dsyId += "_" + (document.getElementById(selectIdArray[index]).selectedIndex - 1);
    };
    var select = document.getElementById(selectIdArray[level]);
    with (select) {
        length = 0;
        options[0] = new Option(defaultValArray[level], defaultValArray[level]);
        if (level && document.getElementById(selectIdArray[level - 1]).selectedIndex > 0 || !level) {
            if (dsy.Exists(dsyId)) {
                itemArray = dsy.Items[dsyId];
                for (index = 0; index < itemArray.length; index++) {
                    options[length] = new Option(itemArray[index], itemArray[index]);
                }
                if (level) { options[0].selected = true; }
            }
        }
        if (++level < selectIdArray.length) {
            change(level);
        }
    }
}

var selectIdArray = ["s_province", "s_city", "s_county"];//三个select的name
//var defaultValArray = ["省份", "地级市", "市、县级市"];//初始值
var defaultValArray = ["", "", ""];//初始值
var selectedValArray = ["", "", ""];//选中的值

//初始化函数
function InitArea(province, city, county) {
    selectedValArray = [];
    selectedValArray.push(province);
    selectedValArray.push(city);
    selectedValArray.push(county);

    document.getElementById(selectIdArray[0]).onchange = new Function("InitItems(1)");//省份改变时
    document.getElementById(selectIdArray[1]).onchange = new Function("InitItems(2)");//城市改变时

    InitItems(0);
}

//初始化子项
function InitItems(level) {
    var select = document.getElementById(selectIdArray[level]);
    var dsyId = "0";
    for (var levelIndex = 0; levelIndex < level; levelIndex++) {
        dsyId += "_" + (document.getElementById(selectIdArray[levelIndex]).selectedIndex - 1);
    };
    with (select) {
        length = 0;//清空子项
        options[0] = new Option(defaultValArray[level], defaultValArray[level]);
        if (dsy.Exists(dsyId)) {
            var itemArray = dsy.Items[dsyId];
            var selectedVal = selectedValArray[level];
            for (var itemIndex = 0; itemIndex < itemArray.length; itemIndex++) {
                var currentValIndex = length;
                var currentVal = itemArray[itemIndex];
                options[currentValIndex] = new Option(currentVal, currentVal);
                if (currentVal == selectedVal) {
                    options[currentValIndex].selected = true;//设置选中项
                }
            }
        }
    }
    if (++level < selectIdArray.length) {
        InitItems(level);//更新下层地市选项
    }
}
